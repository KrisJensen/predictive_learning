...
Can we look at the signal and noise of the change in correlation instead of the the change in weights?
I think this is basically just the projection of the weight change onto the teacher.
...
\begin{align}
    p_0^* &:= p(y_t = y^* | \text{not all correct})\\
    &= \frac{p(\text{not all correct}| y_t = y^*) p(y_t = y^*)}{p(\text{not all correct})}\\
...
\begin{align}
    \dot{\rho} &= \frac{\rho + \dot{\sigma}^2_{|*}}{\sqrt{1+ \dot{\sigma}^2_{||}}} - \rho\\
    &\approx (\rho + \dot{\sigma}^2_{|*})(1-0.5 \dot{\sigma}^2_{||}) - \rho\\
    &\approx \dot{\sigma}^2_{|*} - 0.5 \dot{\sigma}^2_{||},
\end{align}
...
\begin{align}
    \frac{d \rho}{d t} &\approx a \rho + b\\
    \int \frac{1}{a \rho + b} d\rho &= \int dt \\
    \frac{1}{a} \log (a \rho + b) &= t + c\\
    a \rho + b &= c e^{a t} \\
    \rho &= \frac{b}{a} e^{at} - \frac{b}{a}\\
    t(\rho = \lambda) &= \frac{1}{a} \log \left ( 1 + \lambda \frac{a}{b} \right ).
\end{align}
...
\begin{align}
    ...
    \braket{\Delta \b{w}_o} &= \braket{\Delta \b{w}} - \b{w}^T \braket{\Delta \b{w}} \b{w}
\end{align}
Our expected updates all take the form
\begin{equation}
    \braket{\Delta \b{w}} = \alpha_{|} \hat{\b{w}} + \alpha_{*} \hat{\b{w}}^*.
\end{equation}
The orthogonal component is
\begin{align}
    \braket{\Delta \b{w}_o} &= \alpha_{|} \hat{\b{w}} + \alpha_{*} \hat{\b{w}}^* - (\alpha_{|} + \alpha_{*} \rho) \hat{\b{w}}\\
    &= \alpha_{*} (\hat{\b{w}}^* - \rho \hat{\b{w}}).
\end{align}
The change in correlation is given by
\begin{align}
    \dot{\rho} &\approx \b{w}_*^T \Delta \b{w}_o\\
    &= \alpha_{*} (1 - \rho^2).
\end{align}

Empirically, the gradient variance is exactly the same as in the non-orthogonal case.
However, the gradient signal is smaller.
This makes some intuitive sense because the gradients are order 1 times x, but the bit we subtract is multiplied by $\hat{w}$, which has elements of order $N^{-\frac12}$.
